...
void toc_config_parse(TOC_CONN tocc, 
  ...
  for(str=strtok(config,"\n");str;str=strtok(NULL,"\n")) {
	...
	tocc->permit = toc_buddy_group_init("Permit List");
      ...
      toc_send_printf(tocc,"toc_add_permit %s",toc_normalize_username(tmp));
      break;
    case 'd':
	...
	tocc->deny = toc_buddy_group_init("Deny List");
      ...
      toc_send_printf(tocc,"toc_add_deny %s",toc_normalize_username(tmp));
      break;
    case 'm':
      tocc->mode = atol(tmp);
      break;
...
void toc_config_save(TOC_CONN tocc) {
  ...
  printf("DATA: \n%s\n",data);
  toc_send_printf(tocc,"toc_set_config \"%s\"",data);
}
...
TOC_EVENT toc_read_event(TOC_CONN tocc) {
  ...
  if(type == TOC_TYPE_SIGNON) {
    ...
    } else if(!strncmp(tocc->recv_buf,"CONFIG:",7)) {
      /* Config event */
    ...
    } else if(!strncmp(tocc->recv_buf,"IM_IN:",6)) {
      /* Instant message event */
      toce->type = TOC_EV_IM;
      toce->length = len-6;
      strncpy(toce->data,tocc->recv_buf+6,toce->length);
    } else if(!strncmp(tocc->recv_buf,"UPDATE_BUDDY:",13)) {
      /* Update buddy event */
      toce->type = TOC_EV_UPDATE_BUDDY;
      toce->length = len-13;
      strncpy(toce->data,tocc->recv_buf+13,toce->length);
...
VCSI_OBJECT toc_signon(VCSI_CONTEXT vc, 
  ...
  toc_send_printf(tocc, "toc2_login %s %d %s %s %s \"TIC:AIMM\" 160 US \"\" \"\" 3 0 30303 -kentucky -utf8 76144224***",
  ...
  toc_send_text(tocc,"toc_set_caps 09461343-4C7F-11D1-8222-444553540000");
...
VCSI_OBJECT toc_connect(VCSI_CONTEXT vc,
  ...
  if(TOC_DEBUG) printf("TOC ATTEMPTING TO CONNECT\n");
  ...
  if(TOC_DEBUG) printf("TOC CONNECTED - ATTEMPTING SIGNON\n");
...
VCSI_OBJECT toc_im(VCSI_CONTEXT vc,
  ...
  toc_send_printf(TOCC(x),"toc_send_im %s \"%s\"",
